...
export default async function GrowthCentrePage() {
  ...
  const supportedFindings = allFindings.filter(
    (finding) => finding.confidence.level !== "insufficient_data",
  );
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/70 sm:text-base">
                Turn genuine publishing outcomes into cautious next actions.
                Every ranking, pattern and mission comes from recorded posts and
                platform-reported measurements, with confidence shown beside the
                claim rather than hidden behind a growth score.
              </p>
            </div>
        ...
        <section
          aria-label="Growth evidence metrics"
        ...
        >
          ...
          <Metric
            ...
            detail="Published posts with platform observations"
          />
          <Metric
            label="Patterns"
            value={supportedFindings.length}
            detail="Findings above insufficient-data level"
          />
          <Metric
            label="Standouts"
            value={standouts.length}
            detail="Ranked within comparable peer sets"
          />
          <Metric
            label="Repurpose"
            value={repurpose.length}
            detail="Planning candidates, not created posts"
          />
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.55fr)]">
          <div className="rounded-2xl border border-edge bg-panel-raised/35 px-5 py-5 sm:px-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <StatusBadge tone={mission ? "configured" : "inactive"}>
                {mission ? "Evidence available" : "Waiting for evidence"}
              </StatusBadge>
            </div>
            ...
            <div className="mt-5">
              {mission === null ? (
                ...
                <div className="rounded-xl border border-edge bg-panel/40 px-4 py-4">
                  ...
                  <ul className="mt-3 list-disc space-y-1 pl-5 text-[11px] leading-5 text-ink-muted">
          ...
          <div className="rounded-2xl border border-edge bg-panel-raised/35 px-5 py-5">
            ...
            <ul className="mt-4 space-y-3 text-xs leading-5 text-ink-muted">
              <li>Published record ≠ measured performance.</li>
              <li>Observed pattern ≠ proven cause.</li>
              <li>Recommendation ≠ predicted outcome.</li>
              <li>Repurpose candidate ≠ created or scheduled content.</li>
              <li>Experiment observation ≠ randomised causal proof.</li>
            </ul>
          </div>
        </section>
        ...
        <SectionCard
          ...
          description="How much support exists before the dashboard makes a growth statement."
        >
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.6fr)]">
            <div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-edge/70 bg-panel-raised/40 px-4 py-3">
                  <p className="text-xs text-ink-muted">Published posts</p>
                  <p className="mt-1 text-xl font-semibold tabular-nums text-ink-primary">
                    {overview.publishedCount}
                  </p>
                </div>
                <div className="rounded-xl border border-edge/70 bg-panel-raised/40 px-4 py-3">
                  <p className="text-xs text-ink-muted">Measured posts</p>
                  <p className="mt-1 text-xl font-semibold tabular-nums text-ink-primary">
                    {overview.measuredCount}
                  </p>
                </div>
                <div className="rounded-xl border border-edge/70 bg-panel-raised/40 px-4 py-3">
                  <p className="text-xs text-ink-muted">Platforms present</p>
                  <p className="mt-1 text-xl font-semibold tabular-nums text-ink-primary">
            ...
            <div className="rounded-xl border border-edge/70 bg-panel/35 px-4 py-4">
              ...
              <ul className="mt-3 flex flex-col gap-2 text-[11px] leading-5 text-ink-muted">
                {(
                  [
                    "strong_pattern",
                    "moderate_evidence",
        ...
        <SectionCard
          ...
          description={`Ranked by percentile inside its own comparison set using ${METRIC_LABELS.views_or_plays.toLowerCase()}. No synthetic or composite growth score is created.`}
        >
          {standouts.length === 0 ? (
            <EmptyState
              icon={BarChart3}
              title="No ranked standouts yet."
              description="A winner needs enough comparable measured posts to support a ranking. Until then, the dashboard leaves this empty rather than choosing one."
            />
          ) : (
            <ul className="grid gap-3 xl:grid-cols-2">
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          <SectionCard
            title="Topics"
            description="Uses the topic already stored on each content item. It does not classify Scripture or infer a topic from verse text."
          >
            <FindingList
              findings={topicFindings}
              emptyMessage="No topic has enough comparable measured posts yet to support a finding."
            />
          </SectionCard>

          <SectionCard
            title="Length"
            description="Compares existing duration bands without claiming that a platform algorithm prefers a format."
          >
            <FindingList
              findings={durationFindings}
              emptyMessage="Not enough measured posts across different lengths to support a comparison."
            />
          </SectionCard>
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          <SectionCard
            title="Posting time"
            description={`Weekday and local time in ${DEFAULT_TIMEZONE}. Stored UTC instants are converted for analysis.`}
          >
            <div className="flex flex-col gap-5">
              <div>
                ...
                <FindingList
                  ...
                  emptyMessage="Not enough measured posts across different days to support a comparison."
                />
              </div>
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">
                ...
                <FindingList
                  ...
                  emptyMessage="Not enough measured posts across different times to support a comparison."
                />
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard
            ...
            description="Uses deterministic title properties only. No model decides whether a hook is good, viral or persuasive."
          >
            <FindingList
              findings={titleFindings}
              emptyMessage="Not enough measured titled posts to support a comparison."
            />
          </SectionCard>
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          <SectionCard
            title="Goals"
            description="Owner-set targets measured against available observations. Targets are not forecasts."
          >
            <div className="mb-4 flex items-center gap-2 text-xs text-ink-muted">
              <Bullseye aria-hidden="true" className="size-4" />
              Goal progress uses real observations when the metric is available.
            </div>
          ...
          <SectionCard
            title="Experiments"
            description="Observational tests only. The dashboard never upgrades an observation into causal proof."
          >
            <div className="mb-4 flex items-center gap-2 text-xs text-ink-muted">
              <Beaker aria-hidden="true" className="size-4" />
              Conclusions stay human-authored and readiness-gated.
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {experiment.observation ? (
                        <p className="mt-2 rounded-lg border border-edge/60 bg-panel/50 px-3 py-2 text-[11px] leading-5 text-ink-secondary">
                          {experiment.observation}
                        </p>
                      ) : null}
        ...
        <SectionCard
          ...
          description="Evidence-informed planning suggestions only. Nothing here creates a variant, schedules a post, changes approval or publishes content."
        >
          <div className="mb-4 flex items-center gap-2 text-xs text-ink-muted">
            <Repeat2 aria-hidden="true" className="size-4" />
            Candidates appear only after enough measured work exists to rank or
            compare.
          </div>
          ...
          {repurpose.length === 0 ? (
            <EmptyState
              icon={Lightbulb}
              title="No repurpose candidates yet."
              description="Candidates require evidence from already-published work. The Growth Centre does not create speculative opportunities to fill an empty state."
            />
          ) : (
            <ul className="grid gap-3 xl:grid-cols-2">
                ...
                >
                  ...
                  {candidate.contentItemId ? (
                    <Link
                      ...
                      className="mt-3 inline-flex items-center rounded-lg border border-edge-strong bg-panel-raised/60 px-3 py-1.5 text-[11px] font-semibold text-ink-primary transition-colors hover:bg-panel-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-highlight"
                    >
          ...
          <p className="mt-4 text-xs leading-5 text-ink-muted">
            ...
            scheduling remain separate workflows, and human approval remains
